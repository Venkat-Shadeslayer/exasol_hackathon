...
```bash
curl -X POST http://localhost:8000/projects/PROJECT_ID/edit-range \
  -H "content-type: application/json" \
  -d '{"start_time":152.1,"end_time":160.75,"instruction":"Use a simpler example here."}'
```

Natural-language feedback also accepts *"At 3:20 make the graph larger"*, *"Between 5:42 and 6:03 slow down the derivation"*, and direct scene references. Range boundaries use **half-open** intersection semantics, so an edit ending exactly where a scene begins does not invalidate that scene.

Invalidation is scoped to the classified defect:

| Feedback class | Invalidates | Preserves |
|---|---|---|
| Visual defect | code, render, verification | narration, audio |
| Content change | teaching material + downstream scene artifacts | unrelated scenes |
| Narration change | audio, subtitle cues, timing, render, verification | — |

Only active render versions feed the next assembly.

---

## Verification and failure recovery

Verification layers are independent ([src/scholarmotion/verification/](src/scholarmotion/verification/)):

1. AST/import validation and render execution.
2. Instrumented bounds/layout checks.
3. Optional multimodal frame review.
4. Deterministic SymPy and source-grounding checks.
5. Pedagogical objective, cognitive-load, notation, and prerequisite checks.

Repair routing is deterministic and attempts are capped: patch code → regenerate code → revise spec/storyboard → simplify → mark `BLOCKED`. The UI surfaces the last error and repair count rather than silently dropping a scene.

**Generated code is untrusted.** It passes an AST allowlist and runs with a private working directory, minimal environment, timeout, captured output, fixed assets, and no shell invocation. The production worker drops all Linux capabilities and sets `no-new-privileges` (see [docker-compose.yml](docker-compose.yml)). Deployments should additionally place render workers on a network-disabled execution plane while keeping a separate queue consumer.

---

## Correction memory

[knowledge/corrections/correction_file.md](knowledge/corrections/correction_file.md) stores validated recurring failure patterns. Retrieval selects only the top entries matching SceneSpec tags, visual primitives, and prior failures. A content *preference* never becomes correction evidence — defect feedback creates a candidate, and promotion requires repeated, high-confidence evidence plus validation tests.

```bash
python scripts/run_correction_bootstrap.py
python scripts/rebuild_correction_index.py
```

The bootstrap covers equations, transforms, graphs, geometry, vectors, particles, tables, images, camera zoom, subtitles, moving objects, derivations, physics diagrams, and dense layouts — without promoting one-off failures.
...
```bash
python scripts/create_demo.py            # general demo
python scripts/create_emi_demo.py        # electromagnetic induction
python scripts/create_lenz_law_demo.py   # Lenz's law
python scripts/create_kirchhoff_demo.py  # Kirchhoff's laws
```

`create_demo.py` builds an NCERT-level electromagnetic-induction lesson with deterministic providers and simulates a localized content edit plus an overlap defect.
...
| `POST` | `/projects/{id}/sources` | Upload a paper/PDF source |
| `POST` | `/projects/{id}/generate` | Start generation (202) |
| `GET` | `/projects/{id}/progress` | Stage + scene progress |
| `GET` | `/projects/{id}/scenes` | Scene list with status |
| `GET` | `/projects/{id}/video` | Active assembled video |
| `GET` | `/projects/{id}/timeline` | Scene timing + subtitle cues |
| `POST` | `/projects/{id}/feedback` | Natural-language feedback |
| `POST` | `/projects/{id}/edit-range` | Timestamp-range edit |
| `POST` | `/projects/{id}/scenes/{sid}/edit` | Scene-scoped edit |
| `POST` | `/projects/{id}/scenes/{sid}/regenerate` | Force regeneration |
| `GET` | `/projects/{id}/scenes/{sid}/versions` | Version history |
| `POST` | `/projects/{id}/scenes/{sid}/restore/{version}` | Restore a version |
| `GET` | `/corpus/analytics` | Full-corpus coverage aggregations (Exasol) |
| `GET` | `/corpus/health` | Exasol reachability + active scoring path |
| `GET` | `/corrections` | Correction-memory entries |

The live OpenAPI document is at `/docs`.

---

## Project status

Known gaps in this tree, so they are not a surprise:

- **Test coverage is thin.** `pyproject.toml` has always pointed `testpaths` at `tests/`, but no suite was checked in. This tree adds [tests/test_exasol_retrieval.py](tests/test_exasol_retrieval.py) (22 tests over the Exasol scoring/SQL-generation logic, no live server needed); the rest of the pipeline is still uncovered.
- **No `docs/` directory.** Architecture detail lives in this README and in the module docstrings.
- The Makefile's `make worker` passes a `retrieval` queue that has no matching route in [tasks/celery_app.py](src/scholarmotion/tasks/celery_app.py); it is consumed but nothing is routed to it.
- Retrieval falls back silently when Exasol is unreachable, so a build that "works" may not have used it. Confirm with `GET /corpus/health` before claiming a demo ran on Exasol.
